...
function submitSurvey(sessionId,payload) {
  ...
  try {
    ...
    expectedIds.forEach(id=>{
      ...
      const c=block.comments||{}; [['positive','What worked well'],['improve','What should improve'],['additional','Additional comment']].forEach(pair=>{const body=cleanText_(c[pair[0]],4000);if(body)appendRowsLocked_(BCXI.SHEETS.COMPLAINTS,[[uuid_(),submissionId,sessionId,session.respondentId,id,pair[1],body,'SUBMITTED',t]]);});
